...
const Recording = (props: any) => {
  ...
  const [testResponse, setTestResponse] = useState<any>('Waiting...')
  ...
  useEffect(() => {
    ...
    const testBackend = async () => {
      const response = await fetch(process.env.TEST_FETCH || 'http://localhost:3000/test', { mode: 'cors' })
      const data = await response.json()
      console.log('Data:',data)
      if (data && data.message) {
        setTestResponse(data.message)
      } else {
        setTestResponse('No response')
      }
    }
    testBackend()
  }, [])
